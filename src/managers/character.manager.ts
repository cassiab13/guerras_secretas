...
export class CharacterManager implements Manager {
    ...
    public async save(serie: Serie): Promise<void> {
        ...
        const allCharacters: CharacterExternal[] = response.flatMap(response => response.data.results);

        const newCharacters: Character[] = [];
        await Promise.all(allCharacters.map(async (externalCharacter: CharacterExternal) => {
            const character: Character = await this.adapter.toInternalSave(externalCharacter);
            const newCharacter: Character = await this.caching.findCharacter(character);
    
            const saveComic: SaveComicHandler = new SaveComicHandler(newCharacter, externalCharacter.comics as CollectionURI);
            const saveSerie: SaveSerieHandler = new SaveSerieHandler(newCharacter, externalCharacter.series as CollectionURI);
            const saveStorie: SaveStorieHandler = new SaveStorieHandler(newCharacter, externalCharacter.stories as CollectionURI);
            const saveEvent: SaveEventHandler = new SaveEventHandler(newCharacter, externalCharacter.events as CollectionURI);
